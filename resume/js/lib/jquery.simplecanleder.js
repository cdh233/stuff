...
; (function($) {  
    $.simpleCanleder = function(box, options){  
        ...
        var _title_ul_li = ".title .inner>div";   
        box = $(box);  
        ...
        box.click(function(){  
            ...
            canlederBox.css({"top" : boxOffset.top + box_height + 16, "left": boxOffset.left});
        ...
        function scroll(scrollTo){
            var container=canlederBox.find('.title');
        ...
        function _init(){  
            ...
            canlederBox.find(_title_ul_li).html(_getSelect(year));  
            scroll(canlederBox.find(_title_ul_li).find('.on'));
        ...
        function _buildTitle(canlederBox){  
            ...
            //     var year = $select.val();   //$select 在_getSelect()有定�? 
            //     canlederBox.find(_title_ul_li).eq(1).find("div.inner").html(_getSelect(--year));  
            // }).find("div.inner").text(" < ");  
        ...
        function _buildBody(canlederBox){  
            ...
            for(var i = 0; i < 12; i++){  
                ...
                var $li = $("<li/>").append($inner).click(function(){  
                    ...
                    var year = canlederBox.find(_title_ul_li).find('.on').html();

                    console.log(canlederBox.find(_title_ul_li));
        ...
        function _getSelect(year){  
            ...
            var $div=$("<div/>");
            for(var i = 20; i >=0; i--){  
                $div.append($("<div/>").html(year - i ));  
                if(i==0){
                    $div.find('div:last').addClass('on');
                }
            }  
            for(var i = 1; i <= 6; i++){  
                $div.append($("<div/>").text(year + i ));  
            }  
            $div.find('div').click(function(){
                $div.find('div').removeClass('on');
                $(this).addClass('on');
            });
            return $div;
        };  
